(function($){
    ...
    HT.menuRowHtml = (option) => {
        let html 
        let $row = $('<div>').addClass('row menu-item '+ ((typeof(option) != 'undefined') ? option.canonical :'') +'')
        const colums = [
        ...
        let $removeRow = $('<div>').addClass('form-row-center')
        let $a = $('<a>').addClass('delete-menu')
        let $image = $('<img>').attr('src', 'backend/img/icons8-close-52.png')
    ...
    HT.sendAjaxGetMenu = (option, target, menuRowClass) => {
        $.ajax({
            ...
            success:function(res){
                // console.log(res);
                let html = '' // --> html khoi tao ko co gia tri nen gia tri ban dau cua no la undefined sau do cong chuoi duoi 
    ...
    HT.updateNestableOutput = (e) => {
        var list = $(e.currentTarget),output = $(list.data('output'));
