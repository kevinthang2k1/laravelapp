(function($) {
        ...
        if(!$('.wrapper-condition').find('.' + value).elExist()){
            $.ajax({
                ...
                success: function(res){
                    ...
                    //lưu lại khi lỗi dư liệu đối tượng ap dụng lại dữ liệu(*)
    ...
    HT.renderProductAndQuantity = () =>{
        ...
        <div class="choose-module mt20">
            <div class="fix-ladel" style="color:blue" for="" >Sản phẩm áp dụng</div>
        ...
        <table class="table table-striped mt20">
            <thead>
                <tr>
                    ...
                    <th class="text-right" style="width:59px">Tối thiểu</th>
            ...
            <tbody>
                <tr>
                    <td class="chooseProductPromotiomTd">
                        <div 
    ...
    HT.loadProduct = (option) => {
        // console.log(123);
        $.ajax({
            url: 'ajax/product/loadProducPromotion',
            type: 'GET',
            data: option,
            dataType: 'json',
            success: function(res){
    ...
    HT.getPaginationMenu = (option) => {
        $(document).on('click', '.page-link', function(e){
            e.preventDefault()
            let _this = $(this)
            let option = {
                model: _this.parents('.select-product-and-quantity').val(),
                page: _this.text(),
    ...
    HT.fillToObjectList = (data) => {
        switch(data.model){
            ...
            case "ProductCatalogue":
                HT.fillProductCatalogueToLise(data.objects)
                break;
        } 
    }

    HT.fillProductCatalogueToLise = (object) => {
        let html = ''
                ...
                <div class="uk-flex uk-flex-middle uk-flex-space-between">
                    <div class="object-info">
                        <div class="uk-flex uk-flex-middle">
                            ...
                            <div class="object-name">
                                <div class="name">${name}</div>
                                <div class="jscode">Ma san pham:${sku}</div>
                            </div>
                        </div>
                    </div>
    ...
    HT.paginationLinks = (links) => {
        ...
        $.each(links, function(index, link){
            ...
            if(link.label == 'pagination.previous'){
                ...
                let span = $('<a>').addClass('page-link').attr('aria-hidden', true).html('›').attr('data-page', link.label)
                li.append(span)
            }else if(link.url){
