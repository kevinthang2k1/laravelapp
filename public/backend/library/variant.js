(function($){
    ...
    HT.disabledAttributeCatalogueChoose = () => { //không cho chọn nhiều
        let id = [];
    ...
    HT.updateVariantHtml = (variantData) => {
        ...
        html = html + '<input type="checkbox" class="js-switch" data-target="disabled" '+((variantData.variant_filename !== '') ? 'check' : '')+'>'
        html = html + '</div>'
        html = html + '<div class="col-lg-10">'
        html = html + '<div class="row">'
        html = html + '<div class="col-lg-6">'
        html = html + '<label for="" class="control-label">Tên File</label>'
        html = html + '<input type="text" '+((variantData.variant_filename !== '') ? 'checked' : '' )+' name="variant_file_name" value="'+variantData.variant_filename+'" class="form-control '+((variantData.variant_filename !== '') ? 'checked' : '' )+'">'
        html = html + '</div>'
        html = html + '<div class="col-lg-6">'
        html = html + '<label for="" class="control-label">Đường dẫn</label>'
        html = html + '<input type="text" '+((variantData.variant_filename !== '') ? 'checked' : '' )+' name="variant_file_url" value="'+variantData.variant_fileurl+'" class="form-control '+((variantData.variant_filename !== '') ? 'checked' : '' )+'">'
        html = html + '</div>'
        html = html + '</div>'
        html = html + '</div>'
        html = html + '</div>'
    ...
    HT.previewVariantTr = (variant) => {
        ...
        // console.log($('.updateVariantTr').prev().find('.imageSrc'));
    }
    ...
    HT.productVariant = () => {
        ...
        const findIndexVariantBySku = (sku) => data.sku.findIndex((item) => item === sku)
