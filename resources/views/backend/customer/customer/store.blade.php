                ...
                <div class="ibox">
                    ...
                    <div class="ibox-content">
                        ...
                        <div class="row mb15">
                            <div class="col-lg-6 ">
                                <div class="form-row">
                                    ...
                                    <select name="customer_catalogue_id" class="form-control setupSelect2">
                                        ...
                                        <option {{ $item->id  == old('customer_catalogue_id', (isset($customer->customer_catalogue_id)) ? $customer->customer_catalogue_id : '') ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-lg-6">
                                <div class="form-row">
                                    <label for="" class="control-label text-left">Nhóm nguồn khách<span class="text-danger">*</span></label>
                                    <select name="source_id" class="form-control setupSelect2">
                                        <option value="0">[Nhóm nguồn khách]</option>
                                        @foreach($sources as $key => $item)
                                        <option {{ $item->id  == old('source_id', (isset($customer->source_id)) ? $customer->source_id : '') ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <select name="province_id" class="form-control  setupSelect2  province location" data-target="districts">
                                        ...
                                        @if(isset($provinces))
                                            @foreach ($provinces as $provinces)
                                                <option @if(old('province_id') == $provinces-> code ) selected @endif value="{{ $provinces-> code }}">{{ $provinces->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
