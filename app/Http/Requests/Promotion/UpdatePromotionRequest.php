...
use App\Rules\promotion\OrderoAmountRangeRule;
...
class UpdatePromotionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $data = $this->only('startDate','endDate');
        ...
        switch ($method){
            case PromotionEnum::ORDER_AMOUNT_RANGE:
                $rules['method'] = [new OrderoAmountRangeRule($this->input('promotion_order_amount_range'))];
                break;
    ...
    {
        $messages = [
            'name.required' => 'Bạn chưa nhập tên khuyến mại',
        ...
        if(!$this->input('neverEndDate')){
            ...
            $messages['endDate.custom_after'] = 'Ngày kết thúc khuyến mại phải lớn hơn ngày bắt đầu';
        }
        return $messages;
    }
}
