...
class SlideController extends Controller
{
    ...
    public function store(StoreSlideRequest $request){
        ...
        return redirect()->route('slide.admin')->with('error', 'Thêm mới bản ghi không thành công. Hãy thử lại');
    }

    public function edit($id){
        //$this->authorize('modules', 'slide.update');
        $slide = $this->slideRepository->findById($id);
        $slideItems = $this->slideService->converSlideArray($slide->item[$this->language]);
        $config = $this->config();
        $config['seo'] =__('messages.slide');
        $config['method'] = 'edit';
        $template = 'backend.slide.slide.store';
    ...
    public function update($id, UpdateSlideRequest $request){
        ...
        return redirect()->route('slide.admin')->with('error', 'Cập nhật bản ghi không thành công. Hãy thử lại');
    }
    ...
    public function destroy($id){
        ...
        return redirect()->route('slide.admin')->with('error', 'Xóa bản ghi không thành công. Hãy thử lại');
    }
